...
data_sets = np.array(data_set[0][:474]).reshape(1, -1)
data_sets = np.hstack((data_sets, data_set[0][475:].reshape((1, -1))))
data_sets = np.hstack((data_sets, data_set[1:6].reshape(1, -1)))
data_sets = np.hstack((data_sets, data_set[6][:164].reshape(1, -1)))
data_sets = np.hstack((data_sets, data_set[6][165:].reshape(1, -1)))
...
data_labels = np.array(data_label[0][:474]).reshape(1, -1)
data_labels = np.hstack((data_labels, data_label[0][475:].reshape((1, -1))))
data_labels = np.hstack((data_labels, data_label[1:6].reshape(1, -1)))
data_labels = np.hstack((data_labels, data_label[6][:164].reshape(1, -1)))
data_labels = np.hstack((data_labels, data_label[6][165:].reshape(1, -1)))
...
print ("Running Random Train-Test Split over all users...")
Xtrain, Xtest, ytrain, ytest = train_test_split(data_sets, data_labels, test_size=0.25, random_state=5125, stratify=data_labels)
